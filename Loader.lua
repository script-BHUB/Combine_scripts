...
function Library:NewWindow(title)
	...
	function WindowFunctions:NewTab(name, icon)
		...
		function TabFunctions:NewTextBoxInput(placeholder, callback)
			local BoxFrame = Instance.new("Frame", TabContent)
			BoxFrame.Size = UDim2.new(1, -6, 0, 42)
			BoxFrame.BackgroundColor3 = Color3.fromRGB(24, 27, 38)
			BoxFrame.BackgroundTransparency = 0.2
			Instance.new("UICorner", BoxFrame).CornerRadius = UDim.new(0, 6)

			local BoxStroke = Instance.new("UIStroke", BoxFrame)
			BoxStroke.Thickness = 1
			BoxStroke.Color = Color3.fromRGB(35, 40, 60)

			local TextBox = Instance.new("TextBox", BoxFrame)
			TextBox.Size = UDim2.new(1, 0, 1, 0)
			TextBox.BackgroundTransparency = 1
			TextBox.PlaceholderText = placeholder or "ใส่ค่าความเร็ว (เช่น 0.1)..."
			TextBox.PlaceholderColor3 = Color3.fromRGB(90, 95, 110)
			TextBox.Text = ""
			TextBox.TextColor3 = Color3.fromRGB(255, 255, 255)
...
local Window = Library:NewWindow("X-WACK STORE  |  Pisit G-Menu V1")
...
MainTab:NewButton("ล้างโค้ดทั้งหมด (Clear)", function()
	CodeBox.Text = ""
	game:GetService("StarterGui"):SetCore("SendNotification", {
		Title = "X-WACK STORE",
		Text = "เคลียร์ข้อความในช่องวางโค้ดแล้ว!",
		Duration = 2
	})
end)
...
local RunDelay = 0.1
MainTab:NewTextBoxInput("ตั้งค่าดีเลย์ Auto Run (ค่าเดิม: 0.1)", function(value)
	local num = tonumber(value)
	if num and num > 0 then
		RunDelay = num
		game:GetService("StarterGui"):SetCore("SendNotification", {
			Title = "X-WACK STORE",
			Text = "เปลี่ยนดีเลย์เป็น: " .. RunDelay .. " วินาที",
			Duration = 2
		})
	end
end)
